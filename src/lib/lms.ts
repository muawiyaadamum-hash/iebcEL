...
export const REGISTRATION_FEE_EUR = 100;
export const REGISTRATION_FEE_XAF = Math.round(REGISTRATION_FEE_EUR * XAF_TO_EUR);
export function formatEur(amount: number): string {
  return `${amount.toFixed(2).replace(".", ",")} €`;
}
export function xafToEur(xaf: number): number {
  return Math.round((xaf / XAF_TO_EUR) * 100) / 100;
}
